...
public class SetupProcess {
	...
	public @ResponseBody CompanySetupRequestResponse getCompanyDetails(@PathVariable("id") String companyId) {
		...
		try {
				...
				response = commonUtils.createResponseData(companyModal.getDisplayName(), message.GENERALL_FETCH_SUCCESS, message.SUCCESS, "1", companyModal);
				
		} catch (Exception e) {
			
			e.printStackTrace();
			response = commonUtils.createResponseData(e.getMessage(), message.GENERALL_FETCH_FAIL, message.FAIL, "0", companyId);
